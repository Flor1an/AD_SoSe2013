...
public class Baum {
	...
	public void elementEinfuegen(int elem) {
		...
		while (true) {
			...
			if (elem == aktuellerKnoten.getContent()) {
				System.out.println(">  Der Schluessel " + elem + " ist bereits enthalten!");
				return;
			}
			...
			if (elem <= aktuellerKnoten.getContent()) {
				if (aktuellerKnoten.getLinkerSohn() == null) {
					aktuellerKnoten.setLinkerSohn(new Knoten(elem, aktuellerKnoten, null, null, 0));
				...
				if (aktuellerKnoten.getRechterSohn() == null) {
					aktuellerKnoten.setRechterSohn(new Knoten(elem,aktuellerKnoten, null, null, 0));
					balanceFaktor(aktuellerKnoten);
					return;
				} else
	...
	public void elementLoeschen(int elem) {
		...
		try {
			while (true) {
				...
				if (elem < aktuellerKnoten.getContent()) {// kleiner = im linkenteilbaum:
					aktuellerKnoten = aktuellerKnoten.getLinkerSohn();
					
			} else if (elem > aktuellerKnoten.getContent()) {// größer = im rechtenteilbaum:
					aktuellerKnoten = aktuellerKnoten.getRechterSohn();
			}
			}
		} catch (Exception e) {
			System.err.println("Knoten nicht gefunden");
			return;
		}
	...
	private void balanceFaktor(Knoten knoten) {
		...
		if (knoten == wurzel) {
			...
			if (Math.abs(knoten.getBalanceFaktor()) >= 2)
				wurzel = rotiere(knoten);
			return;
		}

		// Balancewerte anpassen:(vereinfacht)
		knoten.setBalanceFaktor((Tiefe(knoten.getRechterSohn()) - Tiefe(knoten.getLinkerSohn())));

		// eventuell Rotieren um die AVL-Bedingung widerherzustellen:
		if (Math.abs(knoten.getBalanceFaktor()) >= 2) {
			knoten = rotiere(knoten);
			//knoten = z;
		}
	 ...
	 */
	public Knoten rotiere(Knoten knoten) {
	 ...
	 */
	private Knoten rechtsRotation(Knoten knoten) {
		System.out.println("Rechtsrotation bei Wert: " + knoten.getContent());
		log.writeToFile("Rechtsrotation bei Wert: " + knoten.getContent());
		
		Knoten linkes = knoten.getLinkerSohn();
		knoten.setLinkerSohn(knoten.getLinkerSohn().getRechterSohn());
		linkes.setRechterSohn(knoten);
		knoten.setBalanceFaktor(Math.max(Tiefe(knoten.getRechterSohn()), Tiefe(knoten.getLinkerSohn())) + 1);
		linkes.setBalanceFaktor(Math.max(Tiefe(linkes.getRechterSohn()), Tiefe(linkes.getLinkerSohn())) + 1);
...
//		try {
//
//			if (knoten.getContent() == knoten.getVaterKnoten().getLinkerSohn().getContent()) { // LINKS
//				linkerSohn = true;
//			} else if (knoten.getContent() == knoten.getVaterKnoten().getRechterSohn().getContent()) {
//				linkerSohn = false;
//			}
//		} catch (Exception e) {
//			// 
...
//		}
	...
	private void linksRotation(Knoten knoten) {
		...
		Knoten rechtes = knoten.getRechterSohn();
		knoten.setRechterSohn(knoten.getRechterSohn().getLinkerSohn());
		rechtes.setLinkerSohn(knoten);
...
//		}
	...
	private void doppelRechtsrotation(Knoten knoten) {
		...
		\     				 /
		 r					l
		 ---------------------------------------*/
		System.out.println("Doppel Rechtsrotation bei Wert: " + knoten.getContent());
		log.writeToFile("Doppel Rechtsrotation bei Wert: " + knoten.getContent());
	...
	private void doppelLinksrotation(Knoten knoten) {
		/*---------------------------------------
		 ...
		 l					        r
		 ---------------------------------------*/
		System.out.println("Doppel Linksrotation bei Wert: " 	+ knoten.getContent());
	...
	public String toInOrdertring() {
		return wurzel.toInOrderString();
	}
	...
	public int summePost() {
		...
		// Knoten) bestimmt.
		return summeTeilbaum(getWurzel().getLinkerSohn()) + summeTeilbaum(getWurzel().getRechterSohn())	+ getKnotenAnzahl();
	}
	...
	public int summePre() {
		...
		return getKnotenAnzahl() + summeTeilbaum(getWurzel().getLinkerSohn()) + summeTeilbaum(getWurzel().getRechterSohn());
	}
}